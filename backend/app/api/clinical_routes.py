...
from app.utils.storage import create_patient as create_patient_local
from app.utils.storage import create_doctor_conversation
from app.utils.storage import create_visit as create_visit_local
from app.utils.storage import get_patient as get_patient_local
from app.utils.storage import get_latest_doctor_conversation
from app.utils.storage import get_latest_intake
from app.utils.storage import get_visit as get_visit_local
...
def _triage_vitals_to_vital_signs(vitals: dict | None) -> list:
    """Convert triage vitals dict to VitalSign-like dicts for SOAP Objective."""
    if not vitals:
        return []
    out: list[dict] = []
    v = vitals
    if v.get("temperature") is not None:
        ...
        out.append({"name": "temperature", "value": str(v["temperature"]), "unit": "°C", "normal_range": "36-37.5", "is_abnormal": abnormal})
    if v.get("bpSystolic") is not None and v.get("bpDiastolic") is not None:
        out.append({"name": "blood_pressure", "value": f"{v['bpSystolic']}/{v['bpDiastolic']}", "unit": "mmHg", "normal_range": "120/80", "is_abnormal": False})
    elif v.get("bpSystolic") is not None:
        out.append({"name": "blood_pressure_systolic", "value": str(v["bpSystolic"]), "unit": "mmHg", "normal_range": "90-120", "is_abnormal": False})
    elif v.get("bpDiastolic") is not None:
        out.append({"name": "blood_pressure_diastolic", "value": str(v["bpDiastolic"]), "unit": "mmHg", "normal_range": "60-80", "is_abnormal": False})
    if v.get("heartRate") is not None:
        ...
        out.append({"name": "heart_rate", "value": str(v["heartRate"]), "unit": "bpm", "normal_range": "60-100", "is_abnormal": abnormal})
    if v.get("respiratoryRate") is not None:
        out.append({"name": "respiratory_rate", "value": str(v["respiratoryRate"]), "unit": "/min", "normal_range": "12-20", "is_abnormal": False})
    if v.get("oxygenSaturation") is not None:
        out.append({"name": "oxygen_saturation", "value": str(v["oxygenSaturation"]), "unit": "%", "normal_range": "95-100", "is_abnormal": False})
    return out
...
async def get_patient_route(
    ...
    _ = auth
    if supabase is None:
        patient = get_patient_local(patient_id)
        if not patient:
...
async def create_visit_route(
    ...
    if supabase is None:
        patient = get_patient_local(payload.patient_id)
        if not patient:
            raise HTTPException(status_code=404, detail=error_payload("NOT_FOUND", "Patient not found", {"patient_id": payload.patient_id}))
        visit_id = str(uuid.uuid4())
        visit = create_visit_local(visit_id=visit_id, patient_id=payload.patient_id, visit_status=payload.visit_status or "open")
        add_audit_log(
            audit_id=str(uuid.uuid4()),
            actor_role=auth.role,
            action="create_visit",
            entity_type="visit",
            entity_id=visit.get("id", "") or visit_id,
...
def get_visit_route(
    ...
    _ = auth
    if supabase is None:
        visit = get_visit_local(visit_id)
        if not visit:
...
def doctor_conversation_route(
    ...
    ai_ok = bool(extract and "structured_data" in extract)
    if ai_ok:
        structured_data: dict = extract["structured_data"]
    else:
        structured_data = {
            "session_id": f"doctor-{visit_id}-{uuid.uuid4()}",
            "extraction_method": "rule_based",
            "demographics": {"age": payload.patient_age, "sex": payload.patient_sex},
            "history": {"past_conditions": [], "current_medications": [], "allergies": [], "family_history": []},
            "chief_complaint": "",
            "symptoms": [],
            "vital_signs": [],
            "clinical_flags": [],
            ...
            "missing_fields": [],
            "extraction_warnings": ["AI engine unavailable"],
    ...
    if triage_vitals:
        ...
        if triage_signs:
            def _key(vs: dict) -> str:
                return str(vs.get("name") or "").lower().replace(" ", "_")

            existing = {_key(vs): vs for vs in (structured_data.get("vital_signs") or [])}
        ...
        if w is not None:
            try:
                structured_data.setdefault("demographics", {})["weight_kg"] = float(w)
            except (TypeError, ValueError):
                pass
        if h is not None:
            try:
                structured_data.setdefault("demographics", {})["height_cm"] = float(h)
            except (TypeError, ValueError):
                pass
        if v.get("bmi") is not None:
            try:
                structured_data.setdefault("demographics", {})["bmi"] = float(v["bmi"])
            except (TypeError, ValueError):
                pass
        # Remove from missing_fields if we now have them
        missing_fields = list(structured_data.get("missing_fields") or [])
        for key in ("weight_kg", "height_cm", "vital_signs"):
            missing_fields = [m for m in missing_fields if m != key]
        structured_data["missing_fields"] = missing_fields

    if ai_ok:
        soap_resp = post_json("/nlp/soap", {"structured_data": structured_data}, timeout_s=30.0)
        soap_note = (soap_resp or {}).get("soap_note")
        if not soap_note:
            ai_ok = False

    if not ai_ok:
        soap_note = {
            "session_id": structured_data.get("session_id") or f"doctor-{visit_id}",
            "subjective": payload.transcript,
            "objective": "",
            "assessment": "AI service unavailable — clinician review required.",
            "plan": "Proceed with clinical assessment.",
            "confidence_level": "low",
            "flags": [],
            "disclaimer": "This SOAP note is AI-generated for support only. It is not a diagnosis or treatment plan.",
            "word_count": len((payload.transcript or "").split()),
        }
        validation = {"is_valid": True, "session_id": structured_data.get("session_id") or "", "errors": [], "warnings": ["AI engine unavailable"], "missing_required": []}
    else:
        validate_resp = post_json("/nlp/validate", {"structured_data": structured_data, "soap_note": soap_note}, timeout_s=30.0)
        validation = (validate_resp or {}).get("validation") or {"is_valid": True, "errors": [], "warnings": []}

    urgency = {"level": "non_urgent", "score": 0, "reasons": ["Urgency assessed from vitals at triage only"], "critical_flags": []}
    ...
    stored = create_doctor_conversation(
        ...
        soap_json={
            ...
            "disclaimer": soap_note.get("disclaimer", ""),
        },
